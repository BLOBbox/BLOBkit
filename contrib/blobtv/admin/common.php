...
function open_db_connection() {
	...
	if ($db_conn = new SQLite3($data_file)) {
		if ($res = @$db_conn->query("SELECT id FROM items LIMIT 1")) {
			// db already exists
			// update db data
		} else {
			// initialize new db
...
function insert_thumb_url($id, $url) {
	...
	return @$db_conn->exec("UPDATE items SET thumbnail = '$url' WHERE id = $id");
}

function insert_video_url($id, $url) {
	global $db_conn;
	$url = $db_conn->escapeString($url);
	return @$db_conn->exec("UPDATE items SET file_name = '$url' WHERE id = $id");
}
...
function aggiorna_video($id, $title, $subtitle, $date, $description) {
	...
	$subtitle = $db_conn->escapeString($subtitle);
	$description = $db_conn->escapeString($description);
	return @$db_conn->exec("UPDATE items SET title = '$title', subtitle = '$subtitle', date_published = '$date', description = '$description' WHERE id = $id");
}

function get_video_details($id) {
	global $db_conn;
	return @$db_conn->querySingle("SELECT * FROM items WHERE id = $id", true);
}
...
function delete_video($id) {
	...
	return @$db_conn->exec("DELETE FROM items WHERE id = $id");
}

function activate_video($id) {
	global $db_conn;
	return $db_conn->exec("UPDATE items SET is_published = 1 WHERE id = $id");
}

function deactivate_video($id) {
	global $db_conn;
	return $db_conn->exec("UPDATE items SET is_published = 0 WHERE id = $id");
}
